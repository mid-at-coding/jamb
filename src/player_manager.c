...
jamb_status_t add_player(const char *dir)
{
	errno = 0;
	FILE *fp = fopen(dir, "r");
	if(!fp){
...
jamb_status_t load_players_from_path(const char *wd)
...
jamb_status_t load_players(state *s)
{
	...
	p_players = vec_player_init();
	if(loaded){
		log_out("Loading players again!", LOG_WARN);
		return JAMB_LOGIC_ERROR;
	}
	jamb_status_t res = load_players_from_path("/etc/jamb/players");
	if(res)
		return res;
	...
	res = load_players_from_path(wd_players);
	if(res)
		return res;
	loaded = true;
	return JAMB_OK;
